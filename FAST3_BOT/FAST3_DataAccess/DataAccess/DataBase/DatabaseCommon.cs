...
namespace FAST3_DataAccess
{
    public class DatabaseCommon
    {
        ...
        public static object GetKeyField<T>()
        {
            ...
            foreach (Attribute attr in objType.GetCustomAttributes(true))
            {
        ...
        public static StringBuilder InsertSql(string tableName, Hashtable ht)
        {
            ...
            foreach (string key in ht.Keys)
            {
                if (ht[key] != null)
                {
                    sb_parame.Append("," + key);
                    sp.Append("," + DbHelper.DbParmChar + "" + key);
                }
            }
            sb.Append(sb_parame.ToString().Substring(1, sb_parame.ToString().Length - 1) + ") Values (");
        ...
        public static StringBuilder UpdateSql(string tableName, Hashtable ht, string pkName)
        {
            ...
            sb.Append(pkName);
            sb.Append("=");
            sb.Append(DbHelper.DbParmChar + pkName);
            return sb;
        }
