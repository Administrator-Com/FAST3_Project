    ...
    public class DatabaseReader
    {
        ...
        public static DataTable ReaderToDataTable(IDataReader dr)
        {
            using (dr)
            {
                ...
                {
                    objDataTable.Columns.Add(dr.GetName(i).ToUpper(), dr.GetFieldType(i));
                }
                objDataTable.BeginLoadData();   //Turns off notifications, index maintenance, and constraints while loading data.
        ...
        public static List<T> ReaderToList<T>(IDataReader dr)
        {
            using (dr)
            {
                ...
                {
                    field.Add(dr.GetName(i).ToUpper());
                }
                ...
                while (dr.Read())
                {
                    ...
                    {
                        if (field.Contains(property.Name.ToUpper()))
                        {
        ...
        public static Hashtable ReaderToHashtable(IDataReader dr)
        {
            using (dr)
            {
                ...
                while (dr.Read())
                {
                    for (int i = 0; i < dr.FieldCount; i++)
                    {
                        string strfield = dr.GetName(i).ToUpper();
